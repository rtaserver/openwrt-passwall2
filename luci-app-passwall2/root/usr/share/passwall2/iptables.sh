...
msg="${msg} except ${TCP_NO_REDIR_PORTS}"
}
[ "$NODE" != "nil" ] && {
msg="TCP default proxy: using node [$(config_n_get $NODE remarks)] [$(get_action_chain_name $TCP_PROXY_MODE)]"
if [ -n "${is_tproxy}" ]; then
...
[ -n "1" ] && {
msg="UDP default proxy: Use node [$(config_n_get $NODE remarks)] [$(get_action_chain_name $UDP_PROXY_MODE)](TPROXY:${REDIR_PORT}) proxy"
