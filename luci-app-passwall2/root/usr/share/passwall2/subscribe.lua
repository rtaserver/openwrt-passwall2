...
-- To obtain the current server of various dynamic configurations, you can use get and set. Get must obtain the node table.
local CONFIG = {}
do
if true then
...
for kk, vv in pairs(CONFIG) do
if (vv.remarks == "Load balancing node list" .. node_id) then
--log("Refresh load balancing node list")
...
--log(result.remarks .. 'Use node AllowInsecure setting: '.. result.tls_allowInsecure)
else
result.tls_allowInsecure = allowInsecure_default and "1" or "0"
end
end
...
--First priority type + remarks + IP + port
if not server then
for index, node in pairs(nodes) do
...
local nodes, szType
local node_list = {}
-- SSD seems to be in this format starting with ssd://
...
local servers = {}
-- SS is wrapped inside, just like this
