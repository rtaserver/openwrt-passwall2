...
msg="Remarks【$remarks】，NFTset【${_ipset}】,"
...
msg2="${msg}Use TCP node [$node_remark] [$(get_action_chain_name $tcp_proxy_mode)]"
if [ -n "${is_tproxy}" ]; then
msg2="${msg2}(TPROXY:${redir_port})proxy"
else
msg2="${msg2}(REDIRECT:${redir_port})proxy"
fi
...
if [ -n "${is_tproxy}" ]; then
msg="${msg}(TPROXY:${REDIR_PORT})Proxy"
else
msg="${msg}(REDIRECT:${REDIR_PORT})Agent"
fi
...
[ -n "1" ] && {
msg="UDP default proxy: Use UDP node [$(config_n_get $NODE remarks)] [$(get_action_chain_name $UDP_PROXY_MODE)](TPROXY:${REDIR_PORT}) proxy"
...
#echolog " - Divert according to request destination address (${proxy_type})..."
...
#echolog "Direct connection between local IPv4 network segments: ${lan_ip}"
...
nft "add rule inet fw4 PSW2_MANGLE meta l4proto tcp iif lo $(REDIRECT $REDIR_PORT TPROXY) comment \"this machine\""
nft "add rule inet fw4 PSW2_MANGLE ip protocol tcp iif lo counter return comment \"this machine\""
...
nft "add rule inet fw4 PSW2_MANGLE_V6 meta l4proto tcp iif lo $(REDIRECT $REDIR_PORT TPROXY) comment \"this machine\""
nft "add rule inet fw4 PSW2_MANGLE_V6 meta l4proto tcp iif lo counter return comment \"this machine\""
}
...
# Handle the shunting or matryoshka of rotation nodes
filter_node $NODE TCP > /dev/null 2>&1 &
filter_node $NODE UDP > /dev/null 2>&1 &
...
nft "add rule inet fw4 PSW2_MANGLE meta l4proto udp iif lo $(REDIRECT $REDIR_PORT TPROXY) comment \"this machine\""
nft "add rule inet fw4 PSW2_MANGLE ip protocol udp iif lo counter return comment \"this machine\""
...
nft "add rule inet fw4 PSW2_MANGLE_V6 meta l4proto udp iif lo $(REDIRECT $REDIR_PORT TPROXY) comment \"this machine\""
nft "add rule inet fw4 PSW2_MANGLE_V6 meta l4proto udp iif lo counter return comment \"this machine\""
fi
