...
[ -z "$(opkg status ${depends} 2>/dev/null | grep 'Status' | awk -F ': ' '{print $2}' 2>/dev/null)" ] && echolog "$tables Start updating rules $depends Not Installed..."
done
else
...
[ $? != 0 ] && {
echolog "-Socks node: [$remarks]${server_host} Is an illegal server address, Unable to start!"
return 1
}
...
msg="${msg} Direct DNS:${DIRECT_DNS_DOH}"
;;
esac
...
echolog "Configure scheduled tasks: every day $time_on Click to enable service."
}
[ -z "$time_restart" -o "$time_restart" != "nil" ] && {
echo "0 $time_restart * * * /etc/init.d/$CONFIG restart" >>/etc/crontabs/root
echolog "Configure scheduled tasks: every day $time_restart Click restart service."
}
fi
...
echolog " - [${remarks}] added to the routing table of interface [${device}] failed! The reason is not found [${device}] gateway."
fi
}
...
#Source $ app_path/helper_dnsmasq.sh add tmp_dnsmasq_path = $ TMP_ACL_PATH/$ signmasq.d dnsmasq_file =/dev/null default_dns = $ Auto_dns tun_dns = 127.0.0.1#$ {dns_port} nftflag = $ {nftflag: -0} no_logic_log =1
...
echolog "The program has been started, please stop and then restart!"
stop
}
...
echolog "fw4 detected, using nftables for transparent proxying."
USE_TABLES="nftables"
nftflag=1
...
DEFAULT_DNS=$(uci show dhcp | grep "@dnsmasq" | grep "\.server=" | awk -F '=' '{print $2}' | sed "s/'//g" | tr ' ' '\ n' | grep -v "\/" | head -2 | sed ':label;N;s/\n/,/;b label')
